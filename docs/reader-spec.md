...
Both paths return estimated in-memory values derived from base files only — `num_records` and `byte_size` are inferred from on-disk sizes using a sampled Parquet footer's compression ratio and average row size. For I/O cost estimation (on-disk base + log file sizes), use `FileSlice::total_size_bytes()` instead.
