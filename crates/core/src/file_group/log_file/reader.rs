...
mod tests {
    ...
    async fn create_log_file_reader(
        ...
        let hudi_configs = HudiConfigs::new([(HudiTableConfig::PrecombineField, "longField")]);
        let storage = Storage::new_with_base_url(dir_url)?;
        LogFileReader::new(Arc::new(hudi_configs), storage, file_name).await
    }
    ...
    async fn test_read_log_file_with_avro_delete_block() -> Result<()> {
        ...
        assert_eq!(block.instant_time()?, "20250209014243389");
