...
mod tests {
    ...
    fn test_valid_filename_parsing() {
        ...
        let log_file = LogFile::try_from(filename).unwrap();
    ...
    fn test_filename_reconstruction() {
        ...
        let log_file = LogFile::try_from(original).unwrap();
        assert_eq!(log_file.file_name(), original);
    }

    #[test]
    fn test_missing_dot_prefix() {
        let filename = "myfile_2024-01-09T12:00:00.log.v1_abc123";
        assert!(matches!(
            LogFile::try_from(filename),
    ...
    fn test_missing_first_underscore() {
        let filename = ".myfile2024-01-09T12:00:00.log.v1_abc123";
        assert!(matches!(
            LogFile::try_from(filename),
            Err(CoreError::FileGroup(_))
        ));
    ...
    fn test_missing_last_underscore() {
        let filename = ".myfile_2024-01-09T12:00:00.log.v1abc123";
        assert!(matches!(
            LogFile::try_from(filename),
            Err(CoreError::FileGroup(_))
        ));
    ...
    fn test_incorrect_dot_parts() {
        let filename = ".myfile_2024-01-09T12:00:00.log.v1.extra_abc123";
        assert!(matches!(
            LogFile::try_from(filename),
            Err(CoreError::FileGroup(_))
        ));
    ...
    fn test_empty_components() {
        let filenames = vec![
            "._2024-01-09T12:00:00.log.v1_abc123",     // empty file_id
            ".myfile_.log.v1_abc123",                  // empty timestamp
            ".myfile_2024-01-09T12:00:00..v1_abc123",  // empty extension
            ".myfile_2024-01-09T12:00:00.log._abc123", // empty version
            ".myfile_2024-01-09T12:00:00.log.v1_",     // empty token
        ];
