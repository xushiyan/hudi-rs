...
impl Table {
    ...
    /// # Errors
    /// Returns an error if:
    /// - The column_stats partition is not available
    /// - The metadata table cannot be read
    ///
    /// # Note
    ...
    pub async fn read_column_stats_for_files(
        ...
        // Read from column_stats partition
        let records = self.read_column_stats_partition(&key_refs).await?;

        // Group by file and convert to StatisticsContainer
        ...
        for record in records {
            ...
            // Convert ColumnStatsRecord to ColumnStatistics
    ...
    /// # Errors
    /// Returns an error if:
    /// - The partition_stats partition is not available
    /// - The metadata table cannot be read
    ///
    /// # Note
    /// Must be called on a DATA table, not a METADATA table.
    pub async fn read_partition_stats(
        ...
        // Read from partition_stats partition
        let records = self.read_partition_stats_partition(&key_refs).await?;

        // Group by partition and convert to StatisticsContainer
        let mut result: HashMap<String, StatisticsContainer> = HashMap::new();
        ...
        for record in records {
            ...
            // Convert PartitionStatsRecord to ColumnStatistics
    ...
    /// Read records from the column_stats partition.
    ///
    /// # Note
    /// Must be called on a DATA table.
    async fn read_column_stats_partition(&self, keys: &[&str]) -> Result<Vec<ColumnStatsRecord>> {
        ...
        for hfile_path in &hfile_paths {
            match read_column_stats_from_hfile_path(&metadata_table, hfile_path, keys).await {
    ...
    /// Read records from the partition_stats partition.
    ///
    /// # Note
    /// Must be called on a DATA table.
    async fn read_partition_stats_partition(
        &self,
        keys: &[&str],
        ...
        let mut all_results = Vec::new();
        for hfile_path in &hfile_paths {
            match read_partition_stats_from_hfile_path(&metadata_table, hfile_path, keys).await {
                Ok(records) => all_results.extend(records),
                Err(e) => {
