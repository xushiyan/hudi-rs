...
use crate::config::table::HudiTableConfig::{KeyGeneratorClass, PartitionFields as PartitionFieldsConfig};
...
impl Table {
    ...
    async fn get_file_slices_internal(
        ...
        // Pass data table if metadata is enabled (for files listing and stats pruning)
        let data_table = if self.is_metadata_table_enabled() {
